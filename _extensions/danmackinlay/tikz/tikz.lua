...
local function configure (meta)
  ...
  if pandoc.system.os == 'windows' then
    local miktex_dir = os.getenv('MIKTEX_BIN') or 'C:/Users/Leung Ho Tin/AppData/Local/Programs/MiKTeX/miktex/bin/x64'
    if tex_engine == 'pdflatex' and io.open(pandoc.path.join { miktex_dir, 'pdflatex.exe' }, 'rb') then
      tex_engine = pandoc.path.join { miktex_dir, 'pdflatex.exe' }
    elseif tex_engine == 'lualatex' and io.open(pandoc.path.join { miktex_dir, 'lualatex.exe' }, 'rb') then
      tex_engine = pandoc.path.join { miktex_dir, 'lualatex.exe' }
    elseif tex_engine == 'xelatex' and io.open(pandoc.path.join { miktex_dir, 'xelatex.exe' }, 'rb') then
      tex_engine = pandoc.path.join { miktex_dir, 'xelatex.exe' }
    end
  end
  ...
  if pandoc.system.os == 'windows' then
    local inkscape_path = 'C:/Program Files/Inkscape/bin/inkscape.exe'
    if svg_engine == 'inkscape' and io.open(inkscape_path, 'rb') then
      svg_engine = inkscape_path
    end
  end
